...
In quantitative finance, a process where volatility scales with price is a lognormal process. When volatility is independent of price, the process is normal (Ho and Lee, 2003). Regression of the price level and squared returns is identified as the method of distinguishing lognormal from normal processes in the literature. However, this simple method of exploring the relationship between price level and volatlity does not provide a satisfactory explanation. Volatility in financial time sereis tends to cluster. 
...
par(mfrow = c(2, 2))
plot(x = wti.df$close, y = wti.df$return**2) + abline(returns.lm, col="red")
...
plot(x = price.regime.descriptive$med, y = price.regime.descriptive$sd, type = "p") + abline(lm(sd ~ med, data = price.regime.descriptive), col = "red")
...
plot(x = price.regime.descriptive.2$med, y = price.regime.descriptive.2$sd, type = "p") + abline(sd.lm, col = "red")
