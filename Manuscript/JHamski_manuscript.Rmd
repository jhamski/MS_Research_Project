...
Where $m$ is the number of change points and their positions in the series at $\tau_{1:m}=(\tau_1,...,\tau_m)$, within a series $y_{1:n} = (y_1,...,y_n)$. $C$ is a cost function for a segement and $\beta f(m)$ is a penalty to increase the threshold to identify a changepoint, thereby preventing overfitting (Killick et al., 2012). Minimizing this calculation can be computationally demanding, hence the development of algorithms such as PELT, however for the oil price series this is not a practial constraint.   
...
```{r fig.cap="Residual Plots of the "}
par(mfrow = c(1, 2))
...
Generalized AutoRegressive Conditional Heteroskedasticity (GARCH) models attempt to model the variance component of a time series. It accounts for asymetric, clustered time series variance and has successfully been applied to model oil prices (Salisu and Fasanya, 2012).  

```{r, eval = T, fig.cap="Condi"}
wti.grch.1 <- wti.combined[,c(1,2)] %>% scale()
...
```{r, eval = F}
wti.grch.2 <- wti.combined[,c(1,5)] %>% scale()
...
# Fit DCC
garchdccfit.2 = dccfit(dcc_spec, wti.grch.2, fit.control=list(scale=TRUE)) 

plot(garchdccfit.2, which = 3)
```
